...
'=================================================
sub ShowHotAtIndex(ArticleNum,TitleLen)
	dim sqlHot,rsHot
	
	dim TDBGColor
	dim sqlArticleSpecialID , rsArticleSpecialID , sqlArticleClassID , rsArticleClassID
	...
	sqlHot=sqlHot & " A.ArticleID,A.Title,A.Author,A.UpdateTime,A.Hits,L.LayoutFileName,A.SpecialID,A.ClassID from article A inner join Layout L on L.LayoutID=A.LayoutID where A.Deleted=False and A.Passed=True And A.Hits>=" & HitsOfHot & " order by A.Hits desc"
	...
	if rsHot.bof and rsHot.eof then 
		response.write "<tr><td background='Image20100223/TDMiddleGreenBackground20100302.jpg'> 无热门文章</td></tr>" 
	else 
		'第一篇文章顯示原文開頭
'Call PaintHotBGColor()
...
'		第一篇完了
		...
		do while not rsHot.eof   
			'显示【文章分类名】
			sqlArticleClassID="select ClassName from ArticleClass where classid=" & rsHot("ClassID")
set			rsArticleClassID = Server.CreateObject("Adodb.RecordSet")
			rsArticleClassID.open sqlArticleClassID,conn,1,3
				 if rsArticleClassID.bof and rsArticleClassID.eof then
				 '无文章分类名的
				 							'开始显示{课程名}
			If rsHot("SpecialID")=0  then '无课程名的
				If TDBGColor=1 then
				response.Write("<tr><td  background='Image20100223/TDMiddleGreenBackground20100302.jpg'>")
				TDBGColor=2
			elseif TDBGColor=2 then 
				response.Write("<tr><td background='Image20100223/TDMiddleRedBackground20100302.jpg'>") 
				TDBGColor=1
			else 
				response.Write("<tr><td  background='Image20100223/TDMiddleGreenBackground20100302.jpg'>")
			end if
			
				response.Write "<p align=left><a href='" & rsHot("LayoutFileName") & "?ArticleID=" & rsHot("articleid") &"' title='文章标题：" & rsHot("Title") & vbcrlf & "作    者：" & rsHot("Author") & vbcrlf & "更新时间：" & rsHot("UpdateTime") & vbcrlf & "点击次数：" & rsHot("Hits") & "' target='_blank'> &nbsp;&nbsp;" & gotTopic(rsHot("title"),TitleLen) & "</a></p><p align=right>[<font color='#000000'>人气" & rsHot("hits") & "</font>]&nbsp;&nbsp; </p>"
			else
			 '有课程名的'无文章分类名的
			    sqlArticleSpecialID="select SpecialAbbreviation from Special where SpecialID=" & rsHot("SpecialID") & ""
				Set rsArticleSpecialID=Server.CreateObject("Adodb.RecordSet")
				rsArticleSpecialID.open sqlArticleSpecialID,conn,1,3
									If TDBGColor=1 then
				response.Write("<tr><td  background='Image20100223/TDMiddleGreenBackground20100302.jpg'>")
				TDBGColor=2
			elseif TDBGColor=2 then 
				...
				response.Write("<tr><td  background='Image20100223/TDMiddleGreenBackground20100302.jpg'>")
									...
									response.Write "<p align=left><a href='" & rsHot("LayoutFileName") & "?ArticleID=" & rsHot("articleid") &"' title='文章标题：" & rsHot("Title") & vbcrlf & "作    者：" & rsHot("Author") & vbcrlf & "更新时间：" & rsHot("UpdateTime") & vbcrlf & "点击次数：" & rsHot("Hits") & "' target='_blank'> &nbsp;&nbsp;" & gotTopic(rsHot("title"),TitleLen) & "</a></p>"
									Response.Write("<p align=right>"  &  "<a href='Article_Special.asp?SpecialID=" &  rsHot("SpecialID")  & "'>[" & rsArticleSpecialID("SpecialAbbreviation") & "]</a>"  & "[<font color='#000000'>人气" & rsHot("hits") & "</font>]&nbsp;&nbsp; </p>")

					
				rsArticleSpecialID.Close
				Set rsArticleSpecialID=Nothing
        	end if '结束显示【课程名】
else '有分类名的，无课程名的
										 							'开始显示{课程名}
			If rsHot("SpecialID")=0  then 
				If TDBGColor=1 then
				response.Write("<tr><td  background='Image20100223/TDMiddleGreenBackground20100302.jpg'>")
				TDBGColor=2
			elseif TDBGColor=2 then 
			...
			elseif TDBGColor=2 then 
				...
				response.Write("<tr><td  background='Image20100223/TDMiddleGreenBackground20100302.jpg'>")
									...
									response.Write "<p align=left><a href='" & rsHot("LayoutFileName") & "?ArticleID=" & rsHot("articleid") &"' title='文章标题：" & rsHot("Title") & vbcrlf & "作    者：" & rsHot("Author") & vbcrlf & "更新时间：" & rsHot("UpdateTime") & vbcrlf & "点击次数：" & rsHot("Hits") & "' target='_blank'> &nbsp;&nbsp;<font size=2 >" & gotTopic(rsHot("title"),TitleLen) & "</font></a></p>"
									Response.Write("<p align=right>"  &   "<a href='Article_Class2.asp?ClassID=" &  rsHot("ClassID")  & "'>[" & rsArticleClassID("ClassName") & "]</a>"   &  "<a href='Article_Special.asp?SpecialID=" &  rsHot("SpecialID")  & "'>[" & rsArticleSpecialID("SpecialAbbreviation") & "]</a>"  & "[<font color='#000000'>人气" & rsHot("hits") & "</font>]&nbsp;&nbsp; </p>")
			...
			rsArticleClassID.close
			set rsArticleClassID=Nothing
'			结束显示{文章分类名】
...
'
sub ShowHotAtIndex_ShowSpecialAbbreviation()
...
sub ShowSpecial(SpecialNum)
	...
	if rsSpecial.bof and rsSpecial.eof then 
		...
		do while not rsSpecial.eof
			response.Write("<img src='images/tree_folder4.gif'><a href='" & rsSpecial(2) & "?SpecialID=" & rsSpecial(0) & "'>" & rsSpecial(1) & "</a></li><br>")
			rsSpecial.movenext
			i=i+1
...
sub ShowArticle_Index(TitleLen_Index,ClassID_Index,SpecialID_Index,MaxPerPage_Index)
	...
	if rsArticle_Index.bof and  rsArticle_Index.eof then
		totalput_Index=0
		response.Write("<li>没有任何符合条件的文章</li>")
	else
...
sub ArticleContent_Index(intTitleLen,ShowProperty,ShowIncludePic,ShowAuthor,ShowDateType,ShowHits,ShowHot,MaxPerPage_Index)
	...
	strTemp=""
		...
		strTemp = strTemp & "<td><img src='images/Star.gif'>&nbsp;<a href='" & rsArticle_Index("LayoutFileName") & "?ArticleID=" & rsArticle_Index("articleid") & "' target='_blank'>"
		...
		strTemp= strTemp & FormatDateTime(rsArticle_Index("UpdateTime"),2) & "</font></td><td align='top'>" 
...
sub ArticleContent(intTitleLen,ShowProperty,ShowIncludePic,ShowAuthor,ShowDateType,ShowHits,ShowHot)
	...
	strTemp=""
		...
		strTemp = strTemp & "<td><img src='images/Star.gif'>&nbsp;<a href='" & rsArticle("LayoutFileName") & "?ArticleID=" & rsArticle("articleid") & "' target='_blank'>"
