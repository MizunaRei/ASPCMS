...
sub SaveArticle()
	...
	'必选老师
	if trim(request.form("TeacherName")) ="" then 
		founderr=true
		errmsg=ErrMsg & "<br><li>必须选择任课教师</li>"
	end if
	if Key="" then
		founderr=true
	...
	if Action="Add" then
		...
		'保存老师
		rs("TeacherName")=trim(request.Form("TeacherName"))
		'结束
		call SaveData()
		rs("Editor")=Editor
		if EnableArticleCheck="No" then
			rs("Passed")=True
			Conn_User.execute("update " & db_User_Table & " set " & db_User_ArticleCount & "=" & db_User_ArticleCount & "+1," & db_User_ArticleChecked & "=" & db_User_ArticleChecked & "+1 where " & db_User_Name & "='" & Editor & "'")
		else
			rs("Passed")=False
			Conn_User.execute("update " & db_User_Table & " set " & db_User_ArticleCount & "=" & db_User_ArticleCount & "+1 where " & db_User_Name & "='" & Editor & "'")
		end if
		rs("OnTop")=False
		rs("Hits")=0
		rs("Hot")=False
