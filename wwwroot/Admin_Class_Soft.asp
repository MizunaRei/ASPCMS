...
<!--#include file="inc/admin_code_soft.asp"-->
<%
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
  <tr class="topbg"> 
    <td height="22" colspan="2" align="center"><strong>软 件 栏 目 管 理</strong></td>
  </tr>
  <tr class="tdbg"> 
    <td width="70" height="30"><strong>管理导航：</strong></td>
    <td height="30"><a href="Admin_Class_Soft.asp">软件栏目管理首页</a> | <a href="Admin_Class_Soft.asp?Action=Add">添加软件栏目</a>&nbsp;|&nbsp;<a href="Admin_Class_Soft.asp?Action=Order">一级栏目排序</a>&nbsp;|&nbsp;<a href="Admin_Class_Soft.asp?Action=OrderN">N级栏目排序</a>&nbsp;|&nbsp;<a href="Admin_Class_Soft.asp?Action=Reset">复位所有软件栏目</a>&nbsp;|&nbsp;<a href="Admin_Class_Soft.asp?Action=Unite">软件栏目合并</a></td>
  </tr>
</table>
<%
if Action="Add" then
...
%>
  <tr class="tdbg" onmouseout="this.style.backgroundColor=''" onmouseover="this.style.backgroundColor='#BFDFFF'"> 
	...
	if iDepth>0 then
	  ...
	  response.write "<a href='Admin_Class_Soft.asp?Action=Modify&ClassID=" & rsClass("ClassID") & "' title='" & rsClass("ReadMe") & "'>" & rsClass("ClassName") & "</a>"
	  if rsClass("Child")>0 then 
    ...
    <td align="center"><a href="Admin_Class_Soft.asp?Action=Add&ParentID=<%=rsClass("ClassID")%>">添加子栏目</a> 
      | <a href="Admin_Class_Soft.asp?Action=Modify&ClassID=<%=rsClass("ClassID")%>">修改设置</a> 
      | <a href="Admin_Class_Soft.asp?Action=Move&ClassID=<%=rsClass("ClassID")%>">移动栏目</a> 
      | <a href="Admin_Class_Soft.asp?Action=Clear&ClassID=<%=rsClass("ClassID")%>" onClick="return ConfirmDel3();">清空</a> 
      | <a href="Admin_Class_Soft.asp?Action=Del&ClassID=<%=rsClass("ClassID")%>" onClick="<%if rsClass("Child")>0 then%>return ConfirmDel1();<%else%>return ConfirmDel2();<%end if%>">删除</a></td>
  </tr>
  <% 
	rsClass.movenext 
loop 
...
{
   if(confirm("删除栏目将同时删除此栏目中的所有软件，并且不能恢复！确定要删除此栏目吗？"))
     return true;
   else
     return false;
...
sub AddClass()
%>
<form name="form1" method="post" action="Admin_Class_Soft.asp" onsubmit="return check()">
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
    <tr class="title"> 
      <td height="22" colspan="2" align="center"><strong>添 加 软 件 栏 目</strong></td>
    </tr>
    <tr class="tdbg"> 
    ...
    <tr class="tdbg"> 
      ...
      <td><%call Admin_ShowLayout_Option(5,LayoutID)%></td>
    </tr>
    <tr class="tdbg"> 
    ...
    <tr class="tdbg"> 
      <td width="350"><strong>栏目发表软件权限：</strong><br>
        只有具有相应权限的人才能在此栏目中发表软件。</td>
    ...
    <tr class="tdbg"> 
      <td height="40" colspan="2" align="center"><input name="Action" type="hidden" id="Action" value="SaveAdd"> 
        ...
        &nbsp; <input name="Cancel" type="button" id="Cancel" value=" 取 消 " onClick="window.location.href='Admin_Class_Soft.asp'" style="cursor:hand;"> 
...
<form name="form1" method="post" action="Admin_Class_Soft.asp" onsubmit="return check()">
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
    <tr class="title"> 
      <td height="22" colspan="2" align="center"><strong>修 改 软 件 栏 目</strong></td>
    </tr>
    <tr class="tdbg"> 
      <td width="350"><strong>所属栏目：</strong><br>
        如果你想改变所属栏目，请<a href='Admin_Class_Soft.asp?Action=Move&ClassID=<%=ClassID%>'>点此移动栏目</a></td>
      <td> <%
	if rsClass("ParentID")<=0 then
	  	response.write "无（作为一级栏目）"
	else
    	dim rsParentClass,sqlParentClass
		sqlParentClass="Select * From SoftClass where ClassID in (" & rsClass("ParentPath") & ") order by Depth"
    ...
    <tr class="tdbg"> 
      <td width="350"><strong>版面设计模板：</strong><br>
        相关模板中包含了栏目设计的版式等信息，如果是自行添加的设计模板，可能会导致“栏目配色模板”失效。 </td>
      <td><%call Admin_ShowLayout_Option(5,rsClass("LayoutID"))%></td>
    </tr>
    <tr class="tdbg"> 
      <td width="350"><strong>栏目图片地址：</strong><br>
        图片会显示在栏目前面。注意图片大小。</td>
...
%>
<form name="form1" method="post" action="Admin_Class_Soft.asp">
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
    <tr class="title"> 
      <td height="22" colspan="2" align="center"><strong>移 动 软 件 栏 目</strong></td>
    </tr>
    <tr class="tdbg"> 
    ...
    <tr class="tdbg"> 
      <td height="40" colspan="2" align="center"><input name="Action" type="hidden" id="Action" value="SaveMove"> 
        ...
        &nbsp; 
        <input name="Cancel" type="button" id="Cancel" value=" 取 消 " onClick="window.location.href='Admin_Class_Soft.asp'" style="cursor:hand;"></td></tr>
  </table>
</form>
<%
	end if
...
<% 
	if j>1 then 
  		response.write "<form action='Admin_Class_Soft.asp?Action=UpOrder' method='post'><td width='150'>" 
	...
	if iCount>j then 
  		response.write "<form action='Admin_Class_Soft.asp?Action=DownOrder' method='post'><td width='150'>" 
...
<% 
	if rsClass("ParentID")>0 then   '如果不是一级栏目，则算出相同深度的栏目数目，得到该栏目在相同深度的栏目中所处位置（之上或者之下的栏目数） 
		...
		if UpMoveNum>0 then 
  			response.write "<form action='Admin_Class_Soft.asp?Action=UpOrderN' method='post'><td width='150'>" 
			response.write "<select name=MoveNum size=1><option value=0>向上移动</option>" 
		...
		if DownMoveNum>0 then 
  			response.write "<form action='Admin_Class_Soft.asp?Action=DownOrderN' method='post'><td width='150'>" 
			response.write "<select name=MoveNum size=1><option value=0>向下移动</option>" 
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border"> 
  <tr class="title">  
    <td height="22" colspan="3" align="center"><strong>复 位 所 有 软 件 栏 目</strong></td> 
  </tr> 
    <tr class="tdbg">  
    <td align="center">  
      <form name="form1" method="post" action="Admin_Class_Soft.asp?Action=SaveReset"> 
        ...
        <input type="submit" name="Submit" value="复位所有栏目"> &nbsp; <input name="Cancel" type="button" id="Cancel" value=" 取 消 " onClick="window.location.href='Admin_Class_Soft.asp'" style="cursor:hand;">
      </form></td>
    </tr>
</table>
<%
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
  <tr class="title"> 
    <td height="22" colspan="3" align="center"><strong>软 件 栏 目 合 并</strong></td>
  </tr>
  <tr class="tdbg"> 
    <td height="100"><form name="myform" method="post" action="Admin_Class_Soft.asp" onSubmit="return ConfirmUnite();">
        &nbsp;&nbsp;将栏目 
  ...
  <tr class="tdbg"> 
    <td height="60"><strong>注意事项：</strong><br>
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;合并后您所指定的栏目（或者包括其下属栏目）将被删除，所有软件将转移到目标栏目中。</td>
  </tr>
</table> 
<script language="JavaScript" type="text/JavaScript">
...
sub SaveAdd()
	...
	set rs = conn.execute("select Max(ClassID) From SoftClass")
	MaxClassID=rs(0)
	...
	set rs=conn.execute("select max(rootid) From SoftClass")
	MaxRootID=rs(0)
	...
	if ParentID>0 then
		sql="select * From SoftClass where ClassID=" & ParentID & ""
    ...
    call CloseConn()
	Response.Redirect "Admin_Class_Soft.asp"  
end sub
...
sub DeleteClass()
	...
	NextID=rs("NextID")
	if rs("Depth")>0 then
		conn.execute("update SoftClass set child=child-1 where ClassID=" & rs("ParentID"))
	end if
	rs.delete
	rs.update
	rs.close
	set rs=nothing
	'删除本栏目的所有软件和评论
	conn.execute("delete from Soft where ClassID=" & ClassID)
	conn.execute("delete from SoftComment where ClassID=" & ClassID)
...
sub ClearClass()
	...
	rs.close
	set rs=nothing
	conn.execute("update Soft set Deleted=True where ClassID in (" & strClassID & ")")
	conn.execute("delete from Soft where ClassID in (" & strClassID & ")")	
	SuccessMsg="此栏目（包括子栏目）的所有软件已经被移到回收站中！"
...
sub SaveMove()
	...
	if rsClass("ParentID")<>rParentID then   '更改了所属栏目，则要做一系列检查
		...
		if rsClass("ParentID")=0 then
			if rParentID>0 then
				set trs=conn.execute("select rootid From SoftClass where LinkUrl='' and ClassID="&rParentID)
...
sub UpOrder()
	...
	sqlOrder="select * From SoftClass where ParentID=0 and RootID<" & cRootID & " order by RootID desc"
	...
	do while not rsOrder.eof
		...
		conn.execute("update SoftClass set RootID=RootID+1 where RootID=" & tRootID)
		i=i+1
		if i>MoveNum then
			rsOrder("PrevID")=ClassID
			rsOrder.update
			conn.execute("update SoftClass set NextID=" & rsOrder("ClassID") & " where ClassID=" & ClassID)
			exit do
		end if
		rsOrder.movenext
	loop
	rsOrder.movenext
	if rsOrder.eof then
		conn.execute("update SoftClass set PrevID=0 where ClassID=" & ClassID)
	else
		rsOrder("NextID")=ClassID
		rsOrder.update
		conn.execute("update SoftClass set PrevID=" & rsOrder("ClassID") & " where ClassID=" & ClassID)
	end if	
	rsOrder.close
	set rsOrder=nothing
	
	'然后再将当前栏目从最后移到相应位置，包括子栏目
	conn.execute("update SoftClass set RootID=" & tRootID & " where RootID=" & MaxRootID)
	call CloseConn()
	response.Redirect "Admin_Class_Soft.asp?Action=Order"
end sub
...
sub DownOrder()
	...
	sqlOrder="select * From SoftClass where ParentID=0 and RootID>" & cRootID & " order by RootID"
	set rsOrder=server.CreateObject("adodb.recordset")
	rsOrder.open sqlOrder,conn,1,3
	if rsOrder.bof and rsOrder.eof then
	...
	rsOrder.movenext
	if rsOrder.eof then
		conn.execute("update SoftClass set NextID=0 where ClassID=" & ClassID)
	else
		rsOrder("PrevID")=ClassID
		rsOrder.update
		conn.execute("update SoftClass set NextID=" & rsOrder("ClassID") & " where ClassID=" & ClassID)
...
sub UpOrderN()
	...
	'要移动的栏目信息
	set rs=conn.execute("select ParentID,OrderID,ParentPath,child,PrevID,NextID From SoftClass where ClassID="&ClassID)
	ParentID=rs(0)
	OrderID=rs(1)
	...
	set rs=nothing
	if child>0 then
		set rs=conn.execute("select count(*) From SoftClass where ParentPath like '%"&ParentPath&"%'")
		oldorders=rs(0)
		rs.close
		set rs=nothing
	else
	...
	sql="select ClassID,OrderID,child,ParentPath,PrevID,NextID From SoftClass where ParentID="&ParentID&" and OrderID<"&OrderID&" order by OrderID desc"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,3
	i=1
	do while not rs.eof
		tOrderID=rs(1)
		conn.execute("update SoftClass set OrderID="&tOrderID+oldorders+i&" where ClassID="&rs(0))
		if rs(2)>0 then
			ii=i+1
			set trs=conn.execute("select ClassID,OrderID From SoftClass where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					conn.execute("update SoftClass set OrderID="&tOrderID+oldorders+ii&" where ClassID="&trs(0))
					ii=ii+1
					trs.movenext
				loop
			end if
		...
		if i>MoveNum then
			rs(4)=ClassID
			rs.update
			conn.execute("update SoftClass set NextID=" & rs(0) & " where ClassID=" & ClassID)		
			exit do
		end if
		rs.movenext
	loop
	...
	if rs.eof then
		...
		rs(5)=ClassID
		rs.update
		conn.execute("update SoftClass set PrevID=" & rs(0) & " where ClassID=" & ClassID)
	end if	
	rs.close
	set rs=nothing
	
	'更新所要排序的栏目的序号
	conn.execute("update SoftClass set OrderID="&tOrderID&" where ClassID="&ClassID)
	'如果有下属栏目，则更新其下属栏目排序
	if child>0 then
		i=1
		set rs=conn.execute("select ClassID From SoftClass where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update SoftClass set OrderID="&tOrderID+i&" where ClassID="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
	...
	call CloseConn()
	response.Redirect "Admin_Class_Soft.asp?Action=OrderN"
end sub
...
sub DownOrderN()
	...
	sql="select ClassID,OrderID,child,ParentPath,PrevID,NextID From SoftClass where ParentID="&ParentID&" and OrderID>"&OrderID&" order by OrderID"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,3
	i=0      '同级栏目
	ii=0     '同级栏目和子栏目
	do while not rs.eof
		conn.execute("update SoftClass set OrderID="&OrderID+ii&" where ClassID="&rs(0))
		if rs(2)>0 then
			set trs=conn.execute("select ClassID,OrderID From SoftClass where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					ii=ii+1
					conn.execute("update SoftClass set OrderID="&OrderID+ii&" where ClassID="&trs(0))
					trs.movenext
				loop
			end if
			trs.close
	...
	'更新所要排序的栏目的序号
	conn.execute("update SoftClass set OrderID="&OrderID+ii&" where ClassID="&ClassID)
	'如果有下属栏目，则更新其下属栏目排序
	if child>0 then
		i=1
		set rs=conn.execute("select ClassID From SoftClass where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update SoftClass set OrderID="&OrderID+ii+i&" where ClassID="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
...
sub SaveReset()
	...
	sql="select ClassID From SoftClass order by RootID,OrderID"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	...
	do while not rs.eof
		...
		rs.moveprevious
		conn.execute("update SoftClass set RootID=" & i & ",OrderID=0,ParentID=0,Child=0,ParentPath='0',Depth=0,PrevID=" & PrevID & ",NextID=" & NextID & " where ClassID=" & rs(0))
		PrevID=rs(0)
		i=i+1
		rs.movenext
	loop
	...
	SuccessMsg="复位成功！请返回<a href='Admin_Class_Soft.asp'>栏目管理首页</a>做栏目的归属设置。"
	call WriteSuccessMsg(SuccessMsg)
end sub
...
sub SaveUnite()
	...
	set rs=conn.execute("select Child from SoftClass where ClassID=" & TargetClassID)
	if rs.bof and rs.eof then
		FoundErr=True
	...
	'得到当前栏目信息
	set rs=conn.execute("select ClassID,ParentID,ParentPath,PrevID,NextID,Depth from SoftClass where ClassID="&ClassID)
	iParentID=rs(1)
	Depth=rs(5)
	if iParentID=0 then
		ParentPath=rs(0)
	...
	'判断是否是合并到其下属栏目中
	set rs=conn.execute("select ClassID from SoftClass where ClassID="&TargetClassID&" and ParentPath like '"&ParentPath&"%'")
	...
	'得到当前栏目的下属栏目ID
	set rs=conn.execute("select ClassID from SoftClass where ParentPath like '"&ParentPath&"%'")
	i=0
	if not (rs.eof and rs.bof) then
		do while not rs.eof
	...
	'更新软件及评论所属栏目
	conn.execute("update Soft set ClassID="&TargetClassID&" where ClassID in ("&ParentPath&")")
	conn.execute("update SoftComment set ClassID="&TargetClassID&" where ClassID in ("&ParentPath&")")
	
	'删除被合并栏目及其下属栏目
	conn.execute("delete from SoftClass where ClassID in ("&ParentPath&")")
