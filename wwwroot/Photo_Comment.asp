...
dim PhotoID,Action,ErrMsg,FoundErr
...
if CommentedID<>"" then
	if instr(CommentedID,"|")>0 then
		...
		for i=0 to ubound(arrCommentedID)
			if Clng(arrCommentedID(i))=PhotoID then
				Commented=True
				exit for
			end if
		next
	else
		if Clng(CommentedID)=PhotoID then
			Commented=True
		end if
	end if
end if
...
<form name="form1" method="post" action="Photo_Comment.asp" onSubmit="return Check();">
  <table width="500" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
    ...
    <tr class="tdbg"> 
      <td align="right" width="110">评论内容：</td>
      <td colspan="3"> 
